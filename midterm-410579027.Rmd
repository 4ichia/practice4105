...
```{r ans132}
subsample <- 
classSurvey[which(classSurvey$isfemale=="TRUE"),]
```
...
hwGrades %>%
  ...
  {rowSums(.,na.rm=T)} -> 總成績 #na.rm=T是讓它當0
hwGrades$平均成績 <- 總成績/3
hwGrades
```
...
```{r ans16}
hwGrades[which(hwGrades$`學號` %in% classSurvey$`學號`),] -> hwGradesSubsample
hwGradesSubsample 
```
...
hwGradesSubsample$平均成績[(hwGradesSubsample$女性==F)] %>%
  mean #已經用錢號$取了平均成績此時資料就不是dataframe了 不會有逗號
#hwGradesSubsample$平均成績[(hwGradesSubsample$女性==T)] %>% mean
```
...
```{r ans19}
str_detect(classSurvey$`課外活動`,"團隊活動")->hasGroupActivity #找出那些人
classSurvey$學號[hasGroupActivity] -> 有團隊的學號

hwGradesSubsample$有團活 <- hwGradesSubsample$學號 %in% 有團隊的學號
hwGradesSubsample$平均成績[(hwGradesSubsample$女性==T & hwGradesSubsample$有團活==T)] %>% #善用true/false
...
```{r ans22}
deadline3 <- ymd_hms("2018-11-06 00:00:00",tz="Asia/Taipei") #記得要放ymd_hms不然直接放字串電腦不知道是時間，且時區tz一定要對
#hwSubmit$Timestamp[(hwSubmit$作業代碼=="Exercise3")] ->hwSubmit3
#deadline3-hwSubmit3
